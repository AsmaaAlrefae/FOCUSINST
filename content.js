(function () {
  ...
  function removeAllInjected() {
    ['focusinst-home-overlay','focusinst-panel','fi-back-home'].forEach(id => document.getElementById(id)?.remove());
  }
  ...
  function applyThemeToAll() {
    ['focusinst-home-overlay','focusinst-panel','fi-back-home'].forEach(id => {
  ...
  function injectBackButton() {
    if (!isEnabled || isHomePage() || document.getElementById('fi-back-home')) return;

    const wrap = document.createElement('div');
    wrap.id = 'fi-back-home';
    wrap.className = themeClass();
    wrap.innerHTML = `
      <div id="fi-back-home-wrap">
        <button id="fi-back-home-btn">
    ...
    document.getElementById('fi-back-home-btn').addEventListener('click', () => { window.location.href = '/'; });
  }
  ...
  function observeDOM() {
    const observer = new MutationObserver(() => {
      ...
      if (!onHome && !document.getElementById('fi-back-home')) injectBackButton();
      if (onHome) document.getElementById('fi-back-home')?.remove();
    ...
    setInterval(() => {
      if (location.pathname !== lastPath) {
        ...
        setTimeout(() => {
          ...
          hideNoiseElements();
          const onHome = isHomePage();
          if (onHome) { injectHomeOverlay(); document.getElementById('fi-back-home')?.remove(); }
          else { document.getElementById('focusinst-home-overlay')?.remove(); injectBackButton(); }
        }, 400);
      }
    }, 300);
  }
